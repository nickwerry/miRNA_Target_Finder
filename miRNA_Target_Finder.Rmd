...
params:
  input_file:
    ...
    value: ""
  subject_genes:
    label: "load file of target genes of interest (TGOIs)"
    input: file
    value: ""
  miRNA_A:
...
#Load set of target genes of interest to filter for later
if (nchar(params$subject_genes[1]) != 0){
  subject_genes <- read_xlsx(params$subject_genes)
} else {
  subject_genes <- NULL
}
...
#Format output
data_output <- if (length(subject_genes) != 0) {
    list(TGOI_Predicted_Result = semi_join(qp_rank_databases, subject_genes, by = "target_symbol"), 
          TGOI_Validated_Result = semi_join(qv_rank, subject_genes, by = "target_symbol"))
  } else if (length(miRNA_confounding) == 0) {
